...
import { Home, BookOpen, Users, ClipboardList, PenTool, Eye, FileText, LogOut, Activity } from 'lucide-react';
...
const hodNav = [
  ...
  { title: 'My Dept Assignments', url: '/hod/my-department', icon: Activity },
  { title: 'Create Paper', url: '/hod/create-paper', icon: PenTool },
  { title: 'Preview Paper', url: '/hod/preview-paper', icon: Eye },
  { title: 'Scheme', url: '/hod/scheme', icon: FileText },
];
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              ...
              <p className="text-[10px] text-sidebar-foreground/70 leading-tight">HOD Portal</p>
            </div>
          )}
        </div>
      </SidebarHeader>
        ...
        <div className="flex-1 flex flex-col min-h-screen">
          <header className="h-14 border-b bg-card flex items-center justify-between px-4 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-muted-foreground hidden sm:inline">HOD / QP Coordinator</span>
            </div>
            <div className="flex items-center gap-3">
              <NotificationDropdown />
              <div className="text-right hidden sm:block">
                <p className="text-sm font-medium text-foreground">{currentUser?.name}</p>
                <p className="text-xs text-muted-foreground">{currentUser?.dept}</p>
              </div>
            </div>
          </header>
